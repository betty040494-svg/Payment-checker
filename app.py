...
def get_main_menu():
    ...
    return QuickReply(items=[
        QuickReplyButton(action=MessageAction(label="👥 對帳管理", text="選單/對帳")),
...
def handle_message(event):
    ...
    # ==========================
    if user_text == "選單/對帳":
        line_bot_api.reply_message(event.reply_token, TextSendMessage(text=f"💡 【對帳管理中心】\n{user_name}，您可以登記幫朋友墊付的東西，或進行多人平分帳單。", quick_reply=get_debt_menu()))
        return
    
    if user_text == "選單/支出":
        line_bot_api.reply_message(event.reply_token, TextSendMessage(text=f"💡 【個人支出管理】\n記下今天的開銷，隨時統計本月花費。", quick_reply=get_expense_menu()))
        return
    ...
    if user_text == "選單/設定":
        ...
        line_bot_api.reply_message(event.reply_token, TextSendMessage(text="💡 【系統設定與說明】\n請設定您的收款資訊或查看指令詳細教學。", quick_reply=setting_menu))
        return
    ...
    if user_text.startswith("墊付/"):
        try:
            ...
            reply = f"✅ 已紀錄對帳明細：\n👤 對象：{name}\n📦 項目：{item}\n💰 金額：{amount} 元"
        except: reply = "⚠️ 格式錯誤！範例：墊付/小明/美式咖啡/55"
    ...
    if user_text.startswith("分帳/"):
        try:
            ...
            reply = f"📝 【{item}】分帳完成！\n每人應付：{avg} 元。\n🏦 收款帳號：\n{bank}"
        except: reply = "⚠️ 範例：分帳/晚餐/小明,小華/1200/10"
        line_bot_api.reply_message(event.reply_token, TextSendMessage(text=reply, quick_reply=get_debt_menu()))
        return
    ...
    if user_text.startswith("支出/"):
        try:
            ...
            reply = f"💰 {user_name}，已記錄個人支出：{p[1]} {p[2]} 元。"
        except: reply = "⚠️ 範例：支出/午餐/100"
    ...
    if user_text.startswith("已收/"):
        ...
        if user_id in USER_DATA['debts'] and name in USER_DATA['debts'][user_id]:
            ...
            reply = f"👌 OK！已結清 {name} 的所有對帳紀錄。"
        else: reply = f"❓ 找不到 {name} 的待收紀錄。"
        line_bot_api.reply_message(event.reply_token, TextSendMessage(text=reply, quick_reply=get_debt_menu()))
        return

    if user_text == "查詢支出":
        exps = USER_DATA['expenses'].get(user_id, [])
        reply = f"📊 {user_name} 本月支出累計：{sum(exps)} 元" if exps else "📭 目前尚無支出紀錄。"
        line_bot_api.reply_message(event.reply_token, TextSendMessage(text=reply, quick_reply=get_expense_menu()))
        return
    ...
    if user_text.startswith("設定帳號/"):
        try:
            ...
            reply = "✅ 收款帳號設定成功！"
        except: reply = "⚠️ 範例：設定帳號/中國信託/12345678"
        line_bot_api.reply_message(event.reply_token, TextSendMessage(text=reply, quick_reply=get_main_menu()))
        return

    if user_text == "確認帳號":
        bank = USER_DATA['bank'].get(user_id, "⚠️ 尚未設定帳號")
        line_bot_api.reply_message(event.reply_token, TextSendMessage(text=f"您的收款帳號資訊：\n{bank}", quick_reply=get_main_menu()))
        return
